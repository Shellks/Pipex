...
/*   Updated: 2023/04/25 14:18:03 by acarlott         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "../include/pipex_bonus.h"

void	last_process(t_pipe *p, char **envp, char **argv, int i)
{	
	if (pipe(p->pipes) == -1)
...
void	parent_process(t_pipe *p, char **envp, char **argv, int i)
{	
	if (pipe(p->pipes) == -1)
		ft_close_free(p, CLOSE_FILE, FREE_PARENT, ERR_PIPE);
	p->pid = fork ();
